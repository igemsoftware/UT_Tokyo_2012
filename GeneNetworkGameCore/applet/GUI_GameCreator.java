...
class tmpDataForCanvas{
  int type, n;
  double k, s;
}

class MyCanvas1 extends ControlWindowCanvas{
  tmpDataForCanvas tdc;
  
  MyCanvas1(tmpDataForCanvas tdc0){
    tdc = tdc0;
    tdc.type = -1;
  }
...
//    pa.println(tdc.type);
    pa.colorMode(pa.HSB, 100);
    pa.background(0);
    // type==1 : dB/dt = f/((k/A)^n + 1)
    // type==0 : dB/dt = f*(1 - 1/((k/A)^n + 1))
    pa.stroke(100);
    pa.line(300, 300, 300, 200);
    pa.line(300, 300, 500, 300);
    pa.fill(100);
    pa.text("concentration of TF", 400, 310);
    pa.text("strength", 260, 220);
    if(tdc!=null){
      if(tdc.type==0){
        // 300 200
        for(int i=0; i<200-1; i++){
          ...
          double a2 = 0.005*(i+1);
          pa.stroke(100);
          pa.line(300+i, 300-(float)(70*tdc.s*(1.0-1.0/(Math.pow((tdc.k/a), tdc.n) + 1.0))),
              301+i, 300-(float)(70*tdc.s*(1.0-1.0/(Math.pow((tdc.k/a2), tdc.n) + 1.0))));
        }
      }else if(tdc.type==1){
        for(int i=0; i<200-1; i++){
          double a = 0.005*i;
          double a2 = 0.005*(i+1);
          pa.stroke(100);
          pa.line(300+i, 300-(float)(70*tdc.s*(1.0/(Math.pow((tdc.k/a), tdc.n) + 1.0))),
              301+i, 300-(float)(70*tdc.s*(1.0/(Math.pow((tdc.k/a2), tdc.n) + 1.0))));
        }
      }
    }
...
//        stg.addInvProm(nlp);
        ...
        win = cp5.addControlWindow("PEditWindow", 100, 100, 520, 400, 15);
        win.setUpdateMode(ControlWindow.NORMAL);
        win.hideCoordinates();
        tdc = new tmpDataForCanvas();
        tdc.type = stg.ps[nlp].type;
        tdc.s = stg.ps[nlp].str;
        tdc.k = stg.ps[nlp].k;
        tdc.n = stg.ps[nlp].n;
        cc1 = new MyCanvas1(tdc);
        cc1.pre();
        win.addCanvas(cc1);
        ...
        */
        ...
        .setWindow(win)
        ;
        cp5.addTextfield("# of genes coded")
        .setPosition(270, 120)
        .setSize(90, 30)
...
//        stg.ps[nlp].type = Integer.parseInt(cp5.get(Textfield.class, "type").getText());
        stg.ps[nlp].type = (int)(cp5.get(RadioButton.class, "type2").getValue());
...
//        cp5.get(Textfield.class, "strength").remove();
        stg.ps[nlp].number = Integer.parseInt(cp5.get(Textfield.class, "# of genes coded").getText());
...
//      stg.ps[stg.NP].type = Integer.parseInt(cp5.get(Textfield.class, "type").getText());
      stg.ps[stg.NP].type = (int)(cp5.get(RadioButton.class, "type2").getValue());
//      cp5.get(Textfield.class, "type").remove();
      stg.ps[stg.NP].str = Double.parseDouble(cp5.get(Textfield.class, "strength").getText());
//      cp5.get(Textfield.class, "strength").remove();
      stg.ps[stg.NP].number = Integer.parseInt(cp5.get(Textfield.class, "# of genes coded").getText());
//      cp5.get(Textfield.class, "# of genes").remove();
...
//      cp5.get(Textfield.class, "n").remove();
    ...
    .setWindow(win)
    ;*/
    cp5.addRadioButton("input_type2")
    .setPosition(20, 280)
    .setSize(40, 20)
    .setItemsPerRow(1)
    .setSpacingColumn(50)
...
//    stg.fs[nlf].input = Integer.parseInt(cp5.get(Textfield.class, "input_type").getText());
    stg.fs[nlf].input = (int)(cp5.get(RadioButton.class, "input_type2").getValue());
...
//    stg.fs[stg.NF].input = Integer.parseInt(cp5.get(Textfield.class, "input_type").getText());
    stg.fs[stg.NF].input = (int)(cp5.get(RadioButton.class, "input_type2").getValue());
  ...
  }else if(e.getController().getName().compareTo("refresh")==0){
        stg.refresh_gc();
  ...
  public void redraw_win(){
    if(cc1!=null){
      try{
//        tdc.type = Integer.parseInt(cp5.get(Textfield.class, "type").getText());
        tdc.type = (int)(cp5.get(RadioButton.class, "type2").getValue());
        tdc.s = Double.parseDouble(cp5.get(Textfield.class, "strength").getText());
        tdc.k = Double.parseDouble(cp5.get(Textfield.class, "k").getText());
        tdc.n = Integer.parseInt(cp5.get(Textfield.class, "n").getText());
      }catch(Exception e){
